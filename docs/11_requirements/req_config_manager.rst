...
.. req:: Runtime Configuration Structure
   ...
   **Description:** The system SHALL maintain a runtime configuration structure in RAM that mirrors compile-time defaults and can be modified via API, with changes taking effect only after system reset.

   **Rationale:** Enables users to modify configuration at runtime without firmware recompilation, while ensuring consistent system state through reset-based application.
   ...
   **Acceptance Criteria:**
   ...
   - AC-5: Configuration changes SHALL take effect only after system reset
...
.. req:: Configuration Save Operation
   ...
   **Description:** The web interface SHALL provide capability to save configuration changes to NVS storage and trigger system reset for consistent application.

   **Rationale:** Users need to persist configuration changes across reboots. System reset ensures all components see consistent configuration state.
   ...
   **Acceptance Criteria:**
   ...
   - AC-5: System SHALL reset within 2 seconds after successful NVS save
   - AC-6: All configuration changes SHALL take effect only after system reset
...
.. req:: Configuration Reset/Reload
   ...
   **Description:** The web interface SHALL provide functionality to reload configuration from NVS or restore factory defaults with appropriate system reset.
   ...
   **Acceptance Criteria:**
   ...
   - AC-6: Factory reset SHALL trigger system reset within 2 seconds
